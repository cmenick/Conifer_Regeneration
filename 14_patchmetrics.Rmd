...
# reasonable model
lm_recovery <- lm(perc_forested ~ perc_core + edge_perim_rat + patch_area + ptch_fr + pet + pet:ptch_fr + ecoregn,
                  data = recovery_df)
summary(lm_recovery)
anova(lm_recovery)

em_recovery <- emmeans::emmeans(lm_recovery,~type_core)
pairs(em_recovery)
```

...
as.data.frame(do.call(rbind,map(list(seg_ndvi_df,seg_ndvi_lp,seg_ndvi_fs), coef))) %>%
  rename(intercept = "(Intercept)",
         slope_post = U1.years_postfire,
         slope_pre = years_postfire) %>% 
  mutate(years_detect = c(as.numeric(seg_ndvi_df$indexU),as.numeric(seg_ndvi_lp$indexU),as.numeric(seg_ndvi_fs$indexU)),
         ...
         years_recover = c(estimate_yrs_recovery(seg_ndvi_df),estimate_yrs_recovery(seg_ndvi_lp),estimate_yrs_recovery(seg_ndvi_fs)))%>% 
  dplyr::select(forest_type, slope_pre,slope_post,adj_R2,years_detect,years_recover)
```

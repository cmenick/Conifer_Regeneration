...
mapview(quadrants_fullset,zcol = "quadrant")
```


```{r}
quadrats_fullset <- head(quadrats_fullset,10)

points_df <- list()
for (j in quadrats_fullset){
  
  patch_fire <- quadrats_fullset %>%
    filter(quad_id == j)
  ...
  points_df[[j]] <- sampled_points
}

points_fullset <- do.call(rbind,points_df)

```

```{r}
st_sample(x,5,type="random")[1:5]
```

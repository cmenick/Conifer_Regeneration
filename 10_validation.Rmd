...
```{r list rast, message=FALSE, warning=FALSE}
...
```{r get props, message=FALSE, warning=FALSE}
...
```{r create pres abs polys, message=FALSE, warning=FALSE}
...
get_polygons <- function(rast_file){
  ...
  mutate(class = case_when(lyr1 == 1 ~ "absence",
                           lyr1 == 2 ~ "presence"),
         Fire_ID = sub( ".*rasters/", "", sub( "_rf_t.*", "", rast_file)))
  return(fire_poly_sf)
}
...
```{r inform sample polys, message=FALSE, warning=FALSE}

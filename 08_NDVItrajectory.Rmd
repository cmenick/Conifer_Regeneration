...
ggplot(ndvi_grouped_type,aes(landsat_yr,delta_ndvi,color=patch_foresttype,group=patch_foresttype))+
  geom_line(size=1) +
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1)) + 
  labs(x="Year",y="Change in NDVI from Pre-Fire",title= "NDVI Difference from Pre-Fire 30 Years Post-Fire",color = "Patch Forest Type") 
```
### NDVI Chaneg by Patch Area
...
ggplot(ndvi_grouped_size,aes(landsat_yr,ndvi,color=patch_area_class,group=patch_area_class))+
  geom_line(size=1) +
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1)) + 
  labs(x="Year",y="NDVI",title= "NDVI Change Overtime 30 Years Post-Fire",color = "Patch Size") 
```

...
```{r message=FALSE, warning=FALSE}
fires_8891 <- fires_select %>%
  filter(year %in% c(1988,1989,1990,1991))

combined<- st_join(fires_8891, fires_8891, join=st_is_within_distance, dist = 10, left = TRUE,remove_self = TRUE) %>% 
  drop_na(fire_name.y) %>% 
  select(fire_name.x,fire_name.y)

overlap <- fires_8891 %>% 
  filter(fire_name %in% combined$fire_name.x)
no_overlap <- fires_8891 %>% 
  filter(!(fire_name %in% combined$fire_name.x))
...
groups$Fire_ID <- str_c("Fire_",c(1:nrow(groups)))
...
no_overlap_groups <- no_overlap %>% 
  mutate(Fire_ID = str_c("Fire_",nrow(groups)+c(1:nrow(no_overlap)))) %>% 
  select(Fire_ID,year)

fires_export <- rbind(joined_overlap_groups,no_overlap_groups) %>% 
  mutate(year=as.integer(year))

mapview(fires_export)
```
...
#st_write(fires_8891, "output/", "fires_8891.shp",driver = 'ESRI Shapefile')
```
